...
module.exports = !__webpack_require__(8) && !__webpack_require__(2)(function () {
  return Object.defineProperty(__webpack_require__(60)('div'), 'a', { get: function get() {
      return 7;
    } }).a != 7;
});
...
// form tabs
var formTabBtns = document.body.querySelectorAll('.form__tabs-nav a');
var formTabs = document.body.querySelectorAll('.form__tabs .form__tabs-item');

formTabBtns.forEach(function (item, index) {
  if (item.parentElement.classList.contains('is-active')) {
    formTabs[index].style.display = 'block';
  }
  item.addEventListener('click', function (e) {
    ...
    if (!item.parentElement.classList.contains('is-active')) {
      ...
      formTabBtns.forEach(function (item) {
...
// rating stars
var rater = __webpack_require__(124);
var raterSatusArr = ['Ужасно', 'Плохо', 'Нормално', 'Хорошо', 'Отлично'];
var rateSatusElem = $('#rate-status');
var statusValInput = $('#rate-status-input');

var myRater = rater({
  element: document.querySelector("#feedBackStars"),
  starSize: 27,
  max: 5,
  min: 0,
  step: 1,
  ...
  rateCallback: function rateCallback(rating, done) {
    ...
    statusValInput.attr('value', rating);
    rateSatusElem.html(raterSatusArr[rating - 1]);
    done();
  }
});
...
scrollToLinks.each(function (index, item) {
  $(item).bind('click', function (e) {
    e.preventDefault();
    var scroollToElem = $(item).attr('href');
    $('html, body').animate({
      scrollTop: $(scroollToElem).offset().top - 10
    }, 1000);
  });
});
...
scrollTopBtn.click(function (ev) {
  ev.preventDefault();
  $('html, body').animate({
...
(function () {
  if ($('.comment-card__answer-slide-cont').length !== 0) {
    var commentCardAnswer = $('.comment-card__answer');

    commentCardAnswer.each(function (index, item) {
      var openAnswerForm = $(item).find('.comment-card__answer-open');
      var answerForm = $(item).find('.comment-card__answer-slide-cont');

      openAnswerForm.click(function (ev) {
        ev.preventDefault();
...
module.exports = !__webpack_require__(87) && !__webpack_require__(120)(function () {
  return Object.defineProperty(__webpack_require__(309)('div'), 'a', { get: function get() {
      return 7;
    } }).a != 7;
});
...
// import './common/lazy-images'
__webpack_require__(127);
// vendor


// common
// import './common/open-modal'
...
var highlightLinks = $('.highlight__anchor-link');
var highlightItem = $('.highlight__item');

highlightLinks.each(function (index, item) {
  $(item).bind('click', function (e) {
    e.preventDefault();
    var scroollToElem = $(item).attr('href');
    $('html, body').animate({
      ...
      if ($(scroollToElem).hasClass('highlight-animate')) {
        $(scroollToElem).removeClass('highlight-animate');
      }
      $(scroollToElem).addClass('highlight-animate');
    });
  });
});

highlightItem.each(function (index, item) {
  $(item).on('animationend webkitAnimationEnd oAnimationEnd', function () {
...
var headerDropdownBtn = $('.header__nav-dropdown-btn');

headerDropdownBtn.each(function (index, item) {
  var closeDropdown = $(item).find('.btn-close');
...
(function () {
  var collapsedRow = $('#collapsed-row');
  var openCollapsed = $('.bottom-links .bottom-links__more');

  openCollapsed.click(function (ev) {
    ev.preventDefault();
    collapsedRow.toggleClass('collapsed');
    if (!collapsedRow.hasClass('collapsed')) {
      openCollapsed.text('Свернуть');
    } else {
      openCollapsed.text('Показать все');
    }
  });
})();
...
(function () {
  if ($('.company-card').lenght != 0) {
    var companyCards = $('.company-card');

    companyCards.each(function (index, item) {
      var comapnyCard = $(item),
          companyPhone = comapnyCard.find('.company-card__phone span'),
          companyShowPhone = comapnyCard.find('.company-card__phone a'),
          comapnyShowMore = comapnyCard.find('.company-card__more');

      // Показать телефон
      companyShowPhone.click(function (ev) {
        ev.preventDefault();
        companyPhone.text(companyPhone.data('fullNumber'));
        companyShowPhone.remove();
      });

      // Развернуть спрятанный блок
      comapnyShowMore.each(function (index, item) {
        $(item).click(function (ev) {
          ev.preventDefault();
          $(item).siblings('.hidden').fadeIn(150);
          $(item).remove();
        });
      });
    });
    ...
    // var starRatingConts = $('.company-card__stars');

    // starRatingConts.each((index, item) => {
    //   var rating = parseFloat($(item).data('rating'));
    //   new Rater({
    //     element: item,
    //     starSize: 18,
    //     max: 5,
...
(function () {
  var subLInksItems = $('.sub-links__item');

  subLInksItems.each(function (index, item) {
    var hiddenBlock = $(item).find('.sub-links__list-hidden');
    var showHiddenBlock = $(item).find('.sub-links__more a');
    var linkLabel = showHiddenBlock.text();

    showHiddenBlock.click(function (ev) {
      ev.preventDefault();
      if (!showHiddenBlock.parent().hasClass('sub-links__more_less')) {
        showHiddenBlock.parent().addClass('sub-links__more_less');
        hiddenBlock.slideDown();
        showHiddenBlock.text('Свернуть');
      } else {
        showHiddenBlock.parent().removeClass('sub-links__more_less');
        hiddenBlock.slideUp();
        showHiddenBlock.text(linkLabel);
      }
    });
  });
})();
...
//   var sortBlockSelects = $('.sort-block__select');
//   sortBlockSelects.each((index, item) => {
//     var selectItem = $(item);
//     selectItem.select2({
//       dropdownParent: $($('.sort-block__modal-cont')[index])
//     });
//   })
// })();
...
(function () {
  var tabHeads = $('.navigator__tabs-head');
  var dropdownItems = $('.navigator__tabs-dropdown');

  var innerTabHeads = $('.navigator__inner-tab-head');
  var innerDropdownItems = $('.navigator__inner-tab-dropdown');

  tabHeads.each(function (index, item) {
    $(item).click(function (ev) {
      $(this).toggleClass('active');
      dropdownItems.eq(index).slideToggle(300);
    });
  });

  innerTabHeads.each(function (index, item) {
    $(item).click(function (ev) {
      $(this).toggleClass('active');
      $(this).siblings(".navigator__inner-tab-dropdown").slideToggle(300);
    });
  });
})();
